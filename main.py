...
def return_comments_dicts(doc_file_path: Path) -> Union[list, bool]:
    ...
    comment was placed, the start character of the comment and the end character of the comment. Then returns a list
    of dictionaries, one dictionary per comment in the file.

    :param doc_file_path: path of a single word document (.docx)
    :return: list of dictionaries, one for each comment
    """
...
def create_annotation_and_text_file(doc_file_path: Path, comments_dicts: list, folders: dict, highlight_mode: bool = True):
    ...
    :param folders: folder structure generated by :func:"create_folder_structure"
    :param highlight_mode: if true, for every comment in the current file, create an ann file with the annotation for
    the text highlighted by the comment. This works better for autoML, else create a single ann file for
    the current file with all the annotations for all the comments
    """
    if highlight_mode:
